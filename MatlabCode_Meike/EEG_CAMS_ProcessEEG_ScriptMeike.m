...
%     CAM_time = table2array(tbl_clin_CAMStime(i_file,2)); CAM_time=num2str(CAM_time);
...
% Extract EEG based features
    ...
    % Save extracted features:
      subjID=subj(i_file);
      file_save = sprintf('%s%cEEG_CAMS_Features_allsegments-subject%s', dir_features, filesep, num2str(subjID));
   ...
   % Log transform some features?
      ...
      file_save = sprintf('%s%cEEG_CAMS_Features_log_allsegments-subject%s', dir_features, filesep, num2str(subjID));
      save(file_save,'features_log_all');
      file_save = sprintf('%s%cEEG_CAMS_Features_log_select_allsegments-subject%s', dir_features, filesep, num2str(subjID));
      save(file_save,'features_log_selection');
      
      %% Reload data per patient
       %cd(dir_features)
      % Find files
       featurefiles = dir('EEG_CAMS_Features_allsegments-subject*.mat');
       featurefileslog=dir('EEG_CAMS_Features_log_allsegments-subject*.mat');
       featurefileslog2=dir('EEG_CAMS_Features_log_select_allsegments-subject*.mat');
...
features_final=[features_avg,features_std,features_min,features_max];          %298*4=1192 features
features_final=array2table(features_final,'VariableNames',feature_names);      %feature table
 
features_final_log=[features_avg_log,features_std_log,features_min_log,features_max_log];
features_final_log=array2table(features_final_log,'VariableNames',feature_names);

features_final_log2=[features_avg_log2,features_std_log2,features_min_log2,features_max_log2];
features_final_log2=array2table(features_final_log2,'VariableNames',feature_names);

file_save = sprintf('features_final', dir_features, filesep);
      save(file_save,'features_final');
file_save = sprintf('features_final_log', dir_features, filesep);
      save(file_save,'features_final_log');
file_save = sprintf('features_final_log2', dir_features, filesep);
      save(file_save,'features_final_log2');

features_final_array=table2array(features_final);
features_final_afterlog=LogTransform(features_final_array);
features_final_afterlog=array2table(features_final_afterlog,'VariableNames',feature_names);

file_save = sprintf('features_final_afterlog', dir_features, filesep);
      save(file_save,'features_final_afterlog');
      
% Save extracted features: features_final (#patient #features)
%       file_save = sprintf('%s%cEEG_CAMS_Features_final_nonICU-subject%s', dir_features, filesep, DateTimestamp());
%       save(file_save);   
...
tbl_clin_3DCAMS=tbl_clin(:,[4, 108]);      % subject ID + 3D-CAMS 

features_final.SubjectID = subj;           %gives EEG table for included patients
...
for i_field = 1:numel(field_out);
    temp_field = field_out{i_field};
    features_final.(temp_field) = nan(size(features_final, 1), 1);
    temp_out = double(tbl_clin_3DCAMS.(temp_field)); % Convert to double in order to have NaN allowable as well
    for i_subj = 1:size(features_final, 1)
        mask_clin_subj = tbl_clin_3DCAMS.SUBJECTID == features_final{i_subj, 'SubjectID'};
        if sum(mask_clin_subj)
           features_final{i_subj, temp_field} = temp_out(mask_clin_subj);
        end
    end
end
...
CAMS_3D=features_final(:,end);         %Outcome measure CAMS (3D)
subjectIDs=features_final(:,end-1);    %Subject IDs

%CAMS_3D=array2table(CAMS_3D);
%tbl_eeg_old=tbl_eeg;                  %With outcome measures included
%tbl_eeg=tbl_eeg(:,1:end-2);           %final eeg feature matrix
%tbl_eeg=array2table(tbl_eeg,'VariableNames',names); %tbl_eeg with feature names
%tbl_eeg.Outcome = tbl_'eeg.(field_out{1});
