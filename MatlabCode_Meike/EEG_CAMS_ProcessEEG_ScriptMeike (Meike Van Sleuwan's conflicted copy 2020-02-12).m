...
for i_file = 30:numel(files)
    ...
    CAM_test_loc = (CAM_time - EEG_start_time)./60;          %in [min]
...
%     data = data(1:end, start_loc:end_loc);
    ...
    lengthdata(s,:)=size(data,2)./Fs./60; % check if works correctly [min]
...
% Extract EEG based features
    ...
    % take the average of the features from all windows   % size(temp_features_avg) = (#features,)
...
% finally, combine the average features from each patient to form a matrix
     ...
     features_avg(s,:)=feature_avg; feature_avg_log(s,:)=feature_avg_log; feature_avg_log2(s,:)=feature_avg_log2;
 ...
 features_final=[features_avg,features_std,features_min,features_max];
 ...
 features_final_log=array2table(features_final_log);
 %names=;
 features_final_log2=[features_avg_log2,features_std_log2,features_min_log2,features_max_log2];
 features_final_log2=array2table(features_final_log2);
 %names=;
...
datatable_clin=tbl_clin(:,[4, 108]); % subject ID + 3D-CAMS
...
CAMS_3D=tbl_eeg(:,end);               %Outcome measure CAMS (3D)
CAMS_3D=array2table(CAMS_3D);
SubjectIDs=tbl_eeg(:,end-1);
SubjectIDs=array2table(SubjectIDs);
