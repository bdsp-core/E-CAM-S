...
function features = EEG_CAMS_FeaturesFromEEG_Meike(bipolar_data, bipolar_abbrev, Fs, filename)
  %bipolar_data is segmental data, doesn't actually use tbl_clin
...
num_sec = size(bipolar_data, 2)/200;
win_samples = size(bipolar_data,2);
frac_overlap = 1 - (2/num_sec);
num_ch=size(bipolar_data,1);
clear p;
for i_ch = 1:num_ch
