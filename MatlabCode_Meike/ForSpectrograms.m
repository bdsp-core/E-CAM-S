...
tbl_clin=tbl_clin(1:200,[1,8,9]);
tbl_clin=tbl_clin(2:end,:);
tbl_clin=cell2table(tbl_clin,'VariableNames',{'SUBJECTID','Date','Time'});
...
tbl_clin1=cell2table(tbl_clin1,'VariableNames',{'SUBJECTID','Date','Time'});
tbl_clin_CAMStimes=tbl_clin1([1:9,11:100,102:134,136:156,158:192,194:end],:); 
%tbl_clin_CAMSscores=
...
CAM_time = table2array(tbl_clin_CAMStimes(:,3)); 
CAM_time=num2str(CAM_time);                               %Clinical test time

CAM_date = table2array(tbl_clin_CAMStimes(:,2)); 
            ...
            datafile=cat(2,datafile,data);
                ...
                datafile=cat(2,datafile,data);
                    ...
                    datafile=cat(2,datafile,data);
                     ...
                     if subj(i_file+5) == subj(i_file+4)
                     filename5=files(i_file+5).name;
                     load(filename6,'data');
                     datafile=cat(2,datafile,data);
                     else
                     data=datafile;
...
%    [Sdata, stimes, sfreqs] = fcn_computeSpec_avg(squeeze(data_wa), params);    %after artefact removal
     [Sdata, stimes, sfreqs] = fcn_computeSpec_avg(resampleddata, params);       %raw data
        ...
        set(gca, 'xtick', 1:3600:length(stimes), 'xticklabel', num2cell(0:1:round(length(stimes)./3600)));
       ...
       cd(dir_eeg2)
       saveas(d,sprintf('Spectrograms_ICU_rawdata_%s.png',filename));
    end
    cla()
    clf()
    close()
    clear stimes Sdata sfreqs set axis
    clear resampleddata
    clear d
    clear set
    clear CAM_test_loc
...
features_final_afterlog_ICU=array2table(features_final_afterlog,'VariableNames',feature_names);
